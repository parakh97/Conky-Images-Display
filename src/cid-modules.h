...
/**
 * Verifie que le fichier de conf d'un plug-in est bien present dans le 
 * repertoire utilisateur du plug-in, sinon le copie a partir du fichier
 * de conf fournit lors de l'installation. Cree au besoin le repertoire 
 * utilisateur du plug-in.
 * @param pVisitCard la carte de visite du module.
 * @return Le chemin du fichier de conf en espace utilisateur, ou NULL 
 * si le fichier n'a pu etre ni trouve, ni cree.
 */
...
/** 
 * Load a module into the table of modules. The module is opened and its
 * visit card and interface are retrieved.
...
*@param erreur error set if something bad happens.
*/
void cid_preload_module_from_directory (const gchar *cModuleDirPath, 
                                        GError **erreur);



void cid_activate_modules_from_list (gchar **cActiveModuleList, 
                                     double fTime);
...
GKeyFile *
cid_pre_read_module_instance_config (CidModuleInstance *pInstance);
...
void cid_reload_module_instance (CidModuleInstance *pInstance, 
                                 gboolean bReloadAppletConf);

/** 
 * Reload all the instances of the module.
 * @param module the module to reload
 * @param bReloadAppletConf TRUE to reload the config of the instances 
...
void 
cid_deactivate_module_instance_and_unload(CidModuleInstance *pInstance);
void cid_deactivate_module_and_unload (const gchar *cModuleName);

void cid_configure_module_instance (GtkWindow *pParentWindow, 
                                    CidModuleInstance *pModuleInstance, 
                                    GError **erreur);
void cid_configure_inactive_module (GtkWindow *pParentWindow, 
                                    CidModule *pModule);
void cid_configure_module (GtkWindow *pParentWindow, 
                           const gchar *cModuleName);
...
CidModule *
cid_foreach_module_in_alphabetical_order (GCompareFunc pCallback, 
                                          gpointer user_data);
...
void 
cid_update_module_instance_order (CidModuleInstance *pModuleInstance, 
                                  double fOrder);


CidModuleInstance *cid_instanciate_module (CidModule *pModule, 
                                           gchar *cConfFilePah);
...
void cid_read_module_config (GKeyFile *pKeyFile, 
                             CidModuleInstance *pInstance);


gboolean cid_reserve_data_slot (CidModuleInstance *pInstance);
void cid_release_data_slot (CidModuleInstance *pInstance);

#define cid_get_icon_data(pIcon, pInstance) \
    ((pIcon)->pDataSlot[pInstance->iSlotID])
#define cid_get_container_data(pContainer, pInstance) \
    ((pContainer)->pDataSlot[pInstance->iSlotID])
...
void cid_reload_internal_module (CidInternalModule *pModule, 
                                 const gchar *cConfFilePath);

CidInternalModule *
cid_find_internal_module_from_name (const gchar *cModuleName);

gboolean cid_get_internal_module_config (CidInternalModule *pModule, 
                                         GKeyFile *pKeyFile);
...
void 
cid_popup_module_instance_description 
(CidModuleInstance *pModuleInstance);


void cid_attach_to_another_module (CidVisitCard *pVisitCard, 
                                   const gchar *cOtherModuleName);

#define cid_module_is_auto_loaded(pModule) \
(pModule->pInterface->initModule == NULL || \
 pModule->pInterface->stopModule == NULL || \
 pModule->pVisitCard->cInternalModule != NULL)
