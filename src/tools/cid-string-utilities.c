...
void
cid_str_replace_all (gchar **string, 
                     const gchar *sFrom, 
                     const gchar *sTo)
{
    ...
    int length = 
        (strlen(*string)+((strlen(sTo)-strlen(sFrom))*size)) * 
        sizeof(gchar)+1;
    g_free (*string);
    *string = NULL;
    *string = g_malloc0(length);
    ...
    cid_datatable_foreach(t_temp,
                          (CidDataAction) cid_datacase_replace,
                          pData);
    cid_clear_datatable(&t_temp);
    g_free (pData[3]);
    g_free (pData);
...
//        return;
    ...
    {
        gchar *from = g_malloc0(2*sizeof(gchar)), 
              *to = g_malloc0(2*sizeof(gchar));
...
{
    CidDataTable *table = 
            cid_create_datatable (CID_TYPE_SUBSTITUTE, 
                                  cid_new_substitute ("%artist%",
                                            musicData.playing_artist ? 
                                            musicData.playing_artist :
                                            ""),
                                  cid_new_substitute ("%album%",
                                            musicData.playing_album ?
                                            musicData.playing_album :
                                            ""),
                                  cid_new_substitute ("%home%",
                                            g_getenv ("HOME")),
                                  cid_new_substitute ("%user%",
                                            g_getenv ("USER")),
                                  G_TYPE_INVALID);
    ...
    pData[0] = GINT_TO_POINTER(0);
    pData[1] = cPath;
    cid_datatable_foreach (table, 
                           (CidDataAction) cid_proceed_substitute, 
                           pData);
...
static void
cid_proceed_regex (gchar **cString, 
                   const gchar *regex, 
...
cid_str_prepare (gchar **cString)
{
    ...
    pData[1] = cString;
    cid_datatable_foreach (table, 
                           (CidDataAction) cid_foreach_proceed_regex, 
                           pData);
    cid_clear_datatable (&table);
    g_free (pData);
}
        ...
        if (*cString == cTokken)
        {
            if (i > 0)
            {
                ...
                cid_datatable_append(&ret, 
                                     cid_datacontent_new_string (cAdd));
                g_free (cTmp);
                g_free (cAdd);
