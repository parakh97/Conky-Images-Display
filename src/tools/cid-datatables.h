...
struct _CidDataContent {
    /// structure anonyme servant à encapsuler un seul 
    /// type de donnée dans une case
    union {
...
/**
 * Fonction permettant de creer une tableau dynamique d'une taille 
 * donnee avec une valeur par defaut donnee
...
#define cid_create_sized_datatable_with_default(iSize,iType,value) \
    cid_create_sized_datatable_with_default_full(iSize, \
                                                 iType, \
                                                 (gpointer)value)
#define cid_create_sized_datatable(iSize) \
    cid_create_sized_datatable_with_default(iSize,G_TYPE_INVALID,NULL)
 ...
 * @return nouvelle liste
 */
void cid_datatable_remove_all(CidDataTable **p_list, 
                              CidDataContent *data);
 ...
 * @return nouvelle liste
 */
void cid_datatable_insert(CidDataTable **p_list, 
                          CidDataContent *data, 
                          gint position);
 ...
 * @param d1 element 1
 * @param d2 element 2
 * @return TRUE si les elements sont egaux (en terme de valeur), 
 * sinon FALSE 
 */
gboolean cid_datacontent_equals (CidDataContent *d1, 
                                 CidDataContent *d2);

/**
 * Permet de parcourir tous les elements d'une liste donnee, 
 * et d'effectue le traitement voulu
 * @param p_list liste que l'on souhaite parcourir
 * @param func pointeur vers la fonction que l'on souhaite effectuer 
 ...
 */
void cid_datatable_foreach (CidDataTable *p_list, 
                            CidDataAction func, 
                            gpointer *pData);
 ...
 * @param pData donnees provenant eventuellement de 
 * #cid_datatable_foreach
 */
 ...
 * @param pData donnees provenant du foreach
 */
void cid_free_datacontent_full (CidDataContent *pContent, 
                                gpointer *pData);
...
#define cid_datacontent_new_string(val) \
    cid_datacontent_new(G_TYPE_STRING, (gpointer)val)
#define cid_datacontent_new_int(val) \
    cid_datacontent_new(G_TYPE_INT, (gpointer)val)
#define cid_datacontent_new_boolean(val) \
    cid_datacontent_new(G_TYPE_BOOLEAN, (gpointer)val)
#define cid_datacontent_new_substitute(val) \
    cid_datacontent_new(CID_TYPE_SUBSTITUTE, (gpointer)val)

#define cid_datatable_remove_first(list) \
    cid_datatable_remove_id(list, 1)
#define cid_datatable_remove_last(list) \
    cid_datatable_remove_id(list, cid_datatable_length(list))
 ...
 */
 CidSubstitute *cid_new_substitute (const gchar *regex, 
