...
{
    return (get_session_connection () != NULL && 
            get_system_connection () != NULL);
}


DBusGProxy *
create_new_session_proxy (const char *name, 
                          const char *path, 
...
DBusGProxy *
create_new_system_proxy (const char *name, 
                         const char *path, 
                         const char *interface) 
{    
...
void 
dbus_call_boolean (DBusGProxy *pDbusProxy, 
