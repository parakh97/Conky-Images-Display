...
void 
_cid_user_action_on_config (GtkDialog *pDialog, 
                            gint action, 
                            gpointer *user_data) 
{
    ...
    {
        // pour prevenir tout interaction avec l'appli pendant sa 
        // re-configuration.
        ...
        else 
        {
            ...
            gchar *cConfiguration = 
                    gtk_text_buffer_get_text (pTextBuffer, 
                                              &start, 
                                              &end, 
                                              FALSE);

            gboolean write_ok = g_file_set_contents (cConfFilePath, 
                                                     cConfiguration, 
                                                     -1, 
                                                     NULL);
    ...
    if (action == GTK_RESPONSE_ACCEPT || 
        action == GTK_RESPONSE_REJECT || 
        action == GTK_RESPONSE_NONE) 
    {
...
_cid_set_color (GtkColorButton *pColorButton, GSList *pWidgetList) 
{
    ...
    if (gtk_color_button_get_use_alpha (pColorButton))
    {
        ...
        gtk_spin_button_set_value (
                pSpinButton, 
                1. * gtk_color_button_get_alpha (pColorButton) / 65535);
    }
}

void 
_cid_get_current_color (GtkColorButton *pColorButton, 
                        GSList *pWidgetList) 
{
    GdkColor gdkColor;
    GtkSpinButton *pSpinButton;
    ...
    if (gtk_color_button_get_use_alpha (pColorButton))
        gtk_color_button_set_alpha (
                    pColorButton, 
                    gtk_spin_button_get_value (pSpinButton) * 65535);
}

gboolean 
_cid_find_iter_from_name (GtkListStore *pModele, 
                          gchar *cName, 
                          GtkTreeIter *iter) 
{
    if (cName == NULL)
    ...
    gtk_tree_model_foreach (GTK_TREE_MODEL (pModele), 
                            (GtkTreeModelForeachFunc)_cid_test_one_name,
                            data);
    return bFound;
}

gboolean 
_cid_test_one_name (GtkTreeModel *model, 
                    GtkTreePath *path, 
                    GtkTreeIter *iter, 
                    gpointer *data) 
{
...
gboolean 
_cid_increase_order (GtkTreeModel *model, 
                     GtkTreePath *path, 
                     GtkTreeIter *iter, 
                     int *pOrder) 
{
...
_cid_go_up (GtkButton *button, GtkTreeView *pTreeView) 
{
    GtkTreeSelection *pSelection = 
                gtk_tree_view_get_selection (pTreeView);
    ...
    gtk_tree_model_foreach (GTK_TREE_MODEL (pModel), 
                            (GtkTreeModelForeachFunc) 
                                    _cid_increase_order, 
                            &iOrder);

    gtk_list_store_set (GTK_LIST_STORE (pModel), 
                        &iter, 
                        CID_MODEL_ORDER, 
                        iOrder, 
                        -1);
...
gboolean 
_cid_decrease_order (GtkTreeModel *model, 
                     GtkTreePath *path, 
                     GtkTreeIter *iter, 
                     int *pOrder) 
{
...
_cid_add (GtkButton *button, gpointer *data) 
{
 ...
 gboolean 
 _cid_decrease_order_if_greater (GtkTreeModel *model, 
                                 GtkTreePath *path, 
                                 GtkTreeIter *iter, 
                                 int *pOrder) 
 {
...
_cid_remove (GtkButton *button, gpointer *data) 
{
    ...
    gtk_tree_model_foreach (GTK_TREE_MODEL (pModel), 
                            (GtkTreeModelForeachFunc) 
                                    _cid_decrease_order_if_greater, 
                            &iOrder);
...
void 
_cid_show_image_preview (GtkFileChooser *pFileChooser, 
                         GtkImage *pPreviewImage) 
{
    gchar *cFileName = 
                gtk_file_chooser_get_preview_filename (pFileChooser);
    if (cFileName == NULL)
        return ;
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_size (cFileName, 
                                                          64, 
                                                          64, 
                                                          NULL);
    g_free (cFileName);
...
_cid_pick_a_file (GtkButton *button, gpointer *data) 
{
    ...
    GtkWidget* pFileChooserDialog = gtk_file_chooser_dialog_new (
        ...
        GTK_RESPONSE_CANCEL,
        NULL);
    gchar *cDirectoryPath = 
                g_path_get_basename (gtk_entry_get_text (pEntry));
    ...
    gtk_file_chooser_set_preview_widget (
                GTK_FILE_CHOOSER (pFileChooserDialog), 
                pPreviewImage);
    g_signal_connect (GTK_FILE_CHOOSER (pFileChooserDialog), 
                      "update-preview", 
                      G_CALLBACK (_cid_show_image_preview), 
                      pPreviewImage);
    ...
    {
        gchar *cFilePath = gtk_file_chooser_get_filename (
                                GTK_FILE_CHOOSER (pFileChooserDialog));
...
{
    const gchar *cFontName = gtk_font_button_get_font_name (
                                    GTK_FONT_BUTTON (widget));
    cid_message (" -> %s", cFontName);
    if (cFontName != NULL)
        gtk_entry_set_text (pEntry, cFontName);
}

void 
_cid_key_grab_cb (GtkWidget *wizard_window, 
                  GdkEventKey *event, 
                  GtkEntry *pEntry) 
{
    gchar *key;
    ...
    if (gtk_accelerator_valid (event->keyval, event->state))
    {
        ...
        *
        * The logic is: keep only the important modifiers that 
        * were pressed for this event. 
        */
        ...
        g_signal_handlers_disconnect_by_func (
                    GTK_OBJECT(wizard_window), 
                    GTK_SIGNAL_FUNC(_cid_key_grab_cb),
                    pEntry);
...
_cid_key_grab_clicked (GtkButton *button, gpointer *data) 
{
    ...
    g_signal_connect (GTK_WIDGET(pParentWindow), 
                      "key-press-event", 
                      GTK_SIGNAL_FUNC(_cid_key_grab_cb), 
                      pEntry);
}
...
void 
_cid_selection_changed (GtkTreeModel *model, 
                        GtkTreeIter iter, 
                        gpointer *data) 
{
    ...
    if (cDescriptionFilePath != NULL)
    {
        ...
        if (strncmp (cDescriptionFilePath, "http://", 7) == 0 || 
            strncmp (cDescriptionFilePath, "ftp://", 6) == 0)
        {
    ...
    if (cPreviewFilePath != NULL)
    {
        ...
        if (strncmp (cPreviewFilePath, "http://", 7) == 0 || 
            strncmp (cPreviewFilePath, "ftp://", 6) == 0)
        {
        ...
        if (gdk_pixbuf_get_file_info (cPreviewFilePath, 
                                      &iPreviewWidth, 
                                      &iPreviewHeight) != NULL)
        {
// Size
            ...
            pPreviewPixbuf = gdk_pixbuf_new_from_file_at_size (
                                        cPreviewFilePath, 
                                        iPreviewWidth, 
                                        iPreviewHeight, 
                                        NULL);
...
gboolean 
_cid_select_one_item_in_tree (GtkTreeSelection *selection, 
                              GtkTreeModel *model, 
                              GtkTreePath *path, 
...
gboolean 
_cid_get_active_elements (GtkTreeModel *model, 
                          GtkTreePath *path, 
                          GtkTreeIter *iter, 
                          GSList **pStringList) 
{
...
_cid_free_widget_list (gpointer *data, gpointer user_data) 
{
    ...
    g_free (data[0]);
    g_free (data[1]);
    // les elements de data[2] sont les widgets, et se feront liberer 
...
void 
cid_update_keyfile_from_widget_list (GKeyFile *pKeyFile, 
                                     GSList *pWidgetList) 
{
    g_slist_foreach (pWidgetList, 
                     (GFunc) _cid_get_each_widget_value, 
                     pKeyFile);
}
...
_cid_get_each_widget_value (gpointer *data, GKeyFile *pKeyFile) 
{
    ...
    if (GTK_IS_CHECK_BUTTON (pOneWidget)) 
    {
        ...
        for (pList = pSubWidgetList; pList != NULL; pList = pList->next) 
        {
            ...
            tBooleanValues[i] = gtk_toggle_button_get_active (
                                        GTK_TOGGLE_BUTTON (pOneWidget));
            i++;
        }
        if (iNbElements > 1)
            g_key_file_set_boolean_list (pKeyFile, 
                                         cGroupName, 
                                         cKeyName, 
                                         tBooleanValues, 
                                         iNbElements);
        else
            g_key_file_set_boolean (pKeyFile, 
                                    cGroupName, 
                                    cKeyName, 
                                    tBooleanValues[0]);
        g_free (tBooleanValues);
             ...
             GTK_IS_HSCALE (pOneWidget)) 
    {
        gboolean bIsSpin = GTK_IS_SPIN_BUTTON (pOneWidget);
        
        if ((bIsSpin && 
             gtk_spin_button_get_digits (GTK_SPIN_BUTTON (pOneWidget)) 
                    == 0) || 
             (!bIsSpin && 
                    gtk_scale_get_digits (GTK_SCALE (pOneWidget)) == 0)) 
        {
            int *tIntegerValues = g_new0 (int, iNbElements);
            for (pList = pSubWidgetList; 
                 pList != NULL; 
                 pList = pList->next) 
            {
                pOneWidget = pList->data;
                tIntegerValues[i] = 
                    (bIsSpin ? 
                        gtk_spin_button_get_value_as_int (
                                GTK_SPIN_BUTTON (pOneWidget)) : 
                        gtk_range_get_value (GTK_RANGE (pOneWidget)));
                i++;
            }
            if (iNbElements > 1)
                g_key_file_set_integer_list (pKeyFile, 
                                             cGroupName, 
                                             cKeyName, 
                                             tIntegerValues, 
                                             iNbElements);
            else
                g_key_file_set_integer (pKeyFile, 
                                        cGroupName, 
                                        cKeyName, 
                                        tIntegerValues[0]);
        ...
        else 
        {
            ...
            if (iNbElements > 1)
                g_key_file_set_double_list (pKeyFile, 
                                            cGroupName, 
                                            cKeyName, 
                                            tDoubleValues, 
                                            iNbElements);
            else
                g_key_file_set_double (pKeyFile, 
                                       cGroupName, 
                                       cKeyName, 
                                       tDoubleValues[0]);
    ...
    else if (GTK_IS_COMBO_BOX (pOneWidget)) 
    {
        ...
        {
            cValue = gtk_combo_box_get_active_text (
                                    GTK_COMBO_BOX (pOneWidget));
        }
        else if (gtk_combo_box_get_active_iter (
                        GTK_COMBO_BOX (pOneWidget), 
                        &iter)) 
        {
        ...
        g_key_file_set_string (pKeyFile, 
                               cGroupName, 
                               cKeyName, 
                               (cValue != NULL ? cValue : "\0"));
        g_free (cValue);
    } else if (GTK_IS_ENTRY (pOneWidget)) {
        gchar *cValue = NULL;
        //gtk_entry_get_text (GTK_ENTRY (pOneWidget));
        const gchar *cWidgetValue = 
                        gtk_entry_get_text (GTK_ENTRY (pOneWidget));
        /*
        ...
        */
        ...
        { 
            // g_key_file_set_locale_string ne marche pas avec une 
            // locale NULL comme le fait 'g_key_file_get_locale_string',
            // il faut donc le faire a la main !
            cKeyNameFull = g_strdup_printf ("%s[%s]", 
                                            cKeyName, 
                                            cPossibleLocales[i]);
            cTranslatedValue = g_key_file_get_string (pKeyFile, 
                                                      cGroupName, 
                                                      cKeyNameFull, 
                                                      NULL);
            g_free (cKeyNameFull);
            if (cTranslatedValue != NULL && 
                strcmp (cTranslatedValue, "") != 0) 
            {
        ...
        if (cPossibleLocales[i] != NULL)
            g_key_file_set_locale_string (pKeyFile, 
                                          cGroupName, 
                                          cKeyName, 
                                          cPossibleLocales[i], 
                                          (cValue!=NULL?cValue : "\0"));
        ...
        GtkTreeModel *pModel = 
                    gtk_tree_view_get_model (GTK_TREE_VIEW(pOneWidget));
        GSList *pActiveElementList = NULL;
        gtk_tree_model_foreach (GTK_TREE_MODEL (pModel), 
                                (GtkTreeModelForeachFunc) 
                                        _cid_get_active_elements, 
                                &pActiveElementList);
        ...
        i = 0;
        GSList * pListElement;
        for (pListElement = pActiveElementList; 
             pListElement != NULL; 
             pListElement = pListElement->next) 
        {
        ...
        if (iNbElements > 1)
            ...
            g_key_file_set_string (pKeyFile, 
                                   cGroupName, 
                                   cKeyName, 
                                   (tStringValues[0] != NULL ? 
                                            tStringValues[0] : 
                                            ""));
        // ses donnees sont dans 'tStringValues' et seront donc 
        // liberees avec.
        g_slist_free (pActiveElementList);  
