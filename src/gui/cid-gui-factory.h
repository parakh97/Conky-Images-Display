...
typedef enum {
    ...
    /// boolean in a button to tick, that will control the sensitivity 
    /// of the next widget.
    ...
    /// list of user dock themes, with a check button to select 
    /// several themes.
    ...
    /// a text entry with a file selector and a 'play' button, 
    /// for sound files.
    ...
    /// a text entry, where text is hidden and the result is encrypted 
    /// in the .conf file.
    ...
    /// a text list.
    CID_WIDGET_LIST='L',
    /// a combo-entry, that is to say a list where one can add a 
    /// custom choice.
    CID_WIDGET_LIST_WITH_ENTRY='E',
    /// a combo where the number of the line is used for the choice.
    CID_WIDGET_NUMBERED_LIST='l',
    /// a combo where the number of the line is used for the choice, 
    /// and for controlling the sensitivity of the widgets below.
    CID_WIDGET_NUMBERED_CONTROL_LIST='y',
    /// a combo where the number of the line is used for the choice, 
    /// and for controlling the sensitivity of the widgets below; 
    /// controlled widgets are indicated in the list : 
    /// {entry;index first widget;nb widgets}.
    CID_WIDGET_NUMBERED_CONTROL_LIST_SELECTIVE='Y',
    /// a tree view, where lines are numbered and can be moved 
    /// up and down.
    CID_WIDGET_TREE_VIEW_SORT='T',
    /// a tree view, where lines can be added, removed, 
    /// and moved up and down.
    CID_WIDGET_TREE_VIEW_SORT_AND_MODIFY='U',
    /// a tree view, where lines are numbered and can be 
    /// selected or not.
    CID_WIDGET_TREE_VIEW_MULTI_CHOICE='V',
    
    /// an empty GtkContainer, to use by applets that want to 
...
gboolean cid_edit_conf_file_with_panel (GtkWindow *pWindow, 
                                        gchar *cConfFilePath, 
                                        const gchar *cTitle, 
                                        int iWindowWidth, 
                                        int iWindowHeight, 
...
gboolean cid_edit_conf_file_core (GtkWindow *pWindow, 
                                  gchar *cConfFilePath, 
                                  const gchar *cTitle, 
                                  int iWindowWidth, 
                                  int iWindowHeight, 
...
GtkWidget *cid_generate_ihm_from_keyfile (GKeyFile *pKeyFile, 
                                          const gchar *cTitle, 
