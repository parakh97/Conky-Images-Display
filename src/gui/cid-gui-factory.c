...
#define _allocate_new_model\
    modele = gtk_list_store_new (CID_MODEL_NB_COLUMNS, \
                                 G_TYPE_STRING, \
                                 G_TYPE_STRING, \
                                 G_TYPE_STRING, \
                                 G_TYPE_BOOLEAN, \
...
#define _pack_in_widget_box(pSubWidget) \
    gtk_box_pack_start (GTK_BOX (pHBox), pSubWidget, FALSE, FALSE, 0)
...
_cid_set_value_in_pair (GtkSpinButton *pSpinButton, gpointer *data)
{
    ...
    if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON(pToggleButton)))
    {
        ...
        int iPairValue = 
            gtk_spin_button_get_value(GTK_SPIN_BUTTON(pPairSpinButton));
        if (iValue != iPairValue)
        {
            gtk_spin_button_set_value (GTK_SPIN_BUTTON(pPairSpinButton),
                                       iValue);
        }
    }
}
...
static void 
_cid_activate_one_element (GtkCellRendererToggle *cell_renderer, 
                           gchar *path, 
                           GtkTreeModel *model)
{
    GtkTreeIter iter;
...
gboolean 
cid_edit_conf_file_with_panel (GtkWindow *pWindow, 
                               gchar *cConfFilePath, 
                               const gchar *cTitle, 
                               int iWindowWidth, 
                               int iWindowHeight, 
                               ...
                               CidReadConfigFunc pConfigFunc, 
                               gchar *cGettextDomain) 
{
    return cid_edit_conf_file_core (pWindow, 
                                    cConfFilePath, 
                                    cTitle, 
                                    iWindowWidth, 
                                    iWindowHeight, 
...
gboolean 
cid_edit_conf_file_core (GtkWindow *pWindow, 
                         gchar *cConfFilePath, 
                         const gchar *cTitle, 
                         int iWindowWidth, 
                         int iWindowHeight, 
                         ...
                         gchar *cGettextDomain) 
{
    ...
    if (cid->pConfigPanel != NULL && cid->runtime->bConfFilePanel) 
    {
        return FALSE;
    }
    cid_message ("%s (%s)", __func__, cConfFilePath);
    GSList *pWidgetList = NULL;
    // le buffer est lie au widget, donc au pDialog.
    ...
    // salete de traducteur automatique.
    g_key_file_remove_key (pKeyFile, 
                           "Desktop Entry", 
    ...
    cid->pConfigPanel = 
        cid_generate_ihm_from_keyfile (pKeyFile, 
                                       cTitle, 
                                       pWindow, 
                                       &pWidgetList, 
                                       (pConfigFunc != NULL) && 
    ...
    if (iWindowWidth != 0 && iWindowHeight != 0)
        gtk_window_resize (GTK_WINDOW (cid->pConfigPanel), 
                           iWindowWidth, 
                           iWindowHeight);
    ...
    g_signal_connect (cid->pConfigPanel, 
                      "response", 
    ...
    if (cid->config->bSafeMode) 
    {
        ...
        g_object_set_data (G_OBJECT (cid->pConfigPanel), 
                           "loop", 
                           pBlockingLoop);
...
GtkWidget *
cid_generate_ihm_from_keyfile (GKeyFile *pKeyFile, 
                               const gchar *cTitle, 
                               GtkWindow *pParentWindow, 
                               GSList **pWidgetList, 
                               gboolean bApplyButtonPresent, 
                               gchar iIdentifier, 
                               gchar *cPresentedGroup, 
                               gchar *cGettextDomain, 
                               GPtrArray *pDataGarbage) 
{
    ...
    gchar *cGroupName, 
          ...
          *cKeyComment, 
          *cUsefulComment, 
          *cAuthorizedValuesChain, 
          *pTipString, 
          **pAuthorizedValuesList, 
          *cSmallGroupIcon;
    ...
    gchar *cOriginalConfFilePath = g_strdup_printf ("%s/%s", 
                                                    CID_DATA_DIR, 
                                                    CID_CONFIG_FILE);
    ...
    {
        pDialog = 
            gtk_dialog_new_with_buttons ((cTitle != NULL ? cTitle : ""),
                NULL,
...
// border size
    ...
    while (pGroupList[i] != NULL) 
    {
        ...
        cGroupComment  = g_key_file_get_comment (pKeyFile, 
                                                 cGroupName, 
                                                 NULL, 
                                                 NULL);
        ...
        pKeyList = g_key_file_get_keys (pKeyFile, 
                                        cGroupName, 
                                        NULL, 
                                        NULL);
        ...
        while (pKeyList[j] != NULL) 
        {
            ...
            cKeyComment =  g_key_file_get_comment (pKeyFile, 
                                                   cGroupName, 
                                                   cKeyName, 
                                                   NULL);
            
            // on gere le bug de la Glib, qui rajoute les nouvelles cles
            // apres le commentaire du groupe suivant !
            if (iElementType == '[')  
            {
                g_free (cKeyComment);
                continue;
            ...
            if (cKeyComment != NULL && strcmp (cKeyComment, "") != 0) 
            {
                ...
                if (pVBox == NULL) 
                { 
                    // maintenant qu'on a au moins un element dans 
                    // ce groupe, on cree sa page dans le notebook.
                    pLabel = gtk_label_new (dgettext (cGettextDomain, 
                                                      cGroupName));
                    
                    pLabelContainer = NULL;
                    GtkWidget *pAlign = NULL;
                    if (cSmallGroupIcon != NULL && 
                        *cSmallGroupIcon != '\0') 
                    {
// marge des box
...
// taille des icones
                    ...
                    pScrolledWindow = gtk_scrolled_window_new (NULL, 
                                                               NULL);
                    ...
                    gtk_notebook_append_page (GTK_NOTEBOOK (pNoteBook), 
                                              ...
                                              (pAlign != NULL ? 
                                                    pAlign : 
                                                    pLabel));
                    if (cPresentedGroup != NULL && 
                        strcmp (cPresentedGroup, cGroupName) == 0)
                ...
                if (*cUsefulComment == '[') 
                {
                    ...
                    *cUsefulComment = '\0';
                    ...
                    pAuthorizedValuesList = 
                            g_strsplit (cAuthorizedValuesChain, ";", 0);
                } 
                else 
                {
...
// border size
                ...
                if (pTipString != NULL) 
                {
                    gchar *pTmpTip = g_strdup (dgettext (cGettextDomain,
                                                         pTipString));
                    if (pTmpTip != NULL)
                    {
                ...
                if (*cUsefulComment != '\0' && 
                    strcmp (cUsefulComment, "...") != 0 && 
                    iElementType != CID_WIDGET_FRAME && 
                    iElementType != CID_WIDGET_EXPANDER && 
                    (iElementType != CID_WIDGET_TESTING || 
                        (iElementType == 't' && 
                         cid->config->bTesting))) 
                {
                        pLabel = gtk_label_new (dgettext(cGettextDomain,
                                                    cUsefulComment));
                        GtkWidget *pAlign = gtk_alignment_new (0., 
                                                               0.5, 
                                                               0., 
                                                               0.);
                        gtk_container_add (GTK_CONTAINER (pAlign), 
                                           pLabel);
                        gtk_box_pack_start ((bIsAligned ? 
                            GTK_BOX (pHBox) : 
                            (pFrameVBox == NULL ? 
                                GTK_BOX (pVBox) : 
                                GTK_BOX (pFrameVBox))),
                ...
                gtk_box_pack_start (pFrameVBox == NULL ? 
                    GTK_BOX (pVBox) : 
                    GTK_BOX (pFrameVBox),
...
// border size
                ...
                switch (iElementType) {
                    ...
                    case CID_WIDGET_CHECK_BUTTON :  // boolean
                        ...
                        length = 0;
                        bValueList = 
                            g_key_file_get_boolean_list (pKeyFile, 
                                                         cGroupName, 
                                                         cKeyName, 
                                                         &length, 
                                                         NULL);
                        ...
                        for (k = 0; k < iNbElements; k ++) 
                        {
                            ...
                            gtk_toggle_button_set_active (
                                    GTK_TOGGLE_BUTTON (pOneWidget), 
                                    bValue);

                            pSubWidgetList = 
                                    g_slist_append (pSubWidgetList, 
                                                    pOneWidget);
                        ...
                        g_free (bValueList);
                    break;
                    // N strings listed from top to bottom.
                    case CID_WIDGET_TREE_VIEW_SORT :  
                    // same with possibility to add/remove some.
                    case CID_WIDGET_TREE_VIEW_SORT_AND_MODIFY :  
                    // N strings that can be selected or not.
                    case CID_WIDGET_TREE_VIEW_MULTI_CHOICE :  
                        ...
                        gtk_tree_view_set_model (
                                GTK_TREE_VIEW (pOneWidget), 
                                GTK_TREE_MODEL (modele));
                        gtk_tree_sortable_set_sort_column_id (
                                GTK_TREE_SORTABLE (modele), 
                                CID_MODEL_ORDER, 
                                GTK_SORT_ASCENDING);
                        ...
                        if (iElementType == 
                            CID_WIDGET_TREE_VIEW_MULTI_CHOICE)
                        {
                        ...
                        pSubWidgetList = g_slist_append (pSubWidgetList,
                                                         pOneWidget);
                        ...
                        if (pAuthorizedValuesList != NULL && 
                            ...
                            for (k = 0; 
                                 pAuthorizedValuesList[k] != NULL; 
                                 k++);
                        else
                            k = 1;
                        gtk_widget_set (pScrolledWindow, 
                                "height-request", 
                                (iElementType == 
                                CID_WIDGET_TREE_VIEW_SORT_AND_MODIFY ? 
                                    100 : 
                                    MIN (100, k * 25)), 
                                NULL);
                        gtk_scrolled_window_set_policy (
                            ...
                            CID_WIDGET_TREE_VIEW_MULTI_CHOICE)
                        {
                            ...
                            pButtonUp = gtk_button_new_from_stock (
                                                GTK_STOCK_GO_UP);
                            ...
                            pButtonDown = gtk_button_new_from_stock (
                                                GTK_STOCK_GO_DOWN);
                            ...
                            CID_WIDGET_TREE_VIEW_SORT_AND_MODIFY)
                        {
                            ...
                            pButtonAdd = gtk_button_new_from_stock (
                                                    GTK_STOCK_ADD);
                            ...
                            gtk_table_attach (GTK_TABLE (pTable),
                                ...
                                0, 0);
                            pButtonRemove = gtk_button_new_from_stock (
                                                    GTK_STOCK_REMOVE);
                        ...
                        if (iElementType == 
                            CID_WIDGET_TREE_VIEW_SORT_AND_MODIFY)  
                        { // on liste les choix actuels tout simplement.
                        ...
                        // on liste les choix possibles dans l'ordre 
                        // choisi. Pour 
                        // CID_WIDGET_TREE_VIEW_MULTI_CHOICE, on 
                        // complete avec ceux n'ayant pas ete 
                        // selectionnes.
                        else if (pAuthorizedValuesList != NULL)  
                        {
                            gint iNbPossibleValues = 0, iOrder = 0;
                            while (
                                pAuthorizedValuesList[iNbPossibleValues]
                                != NULL)
                                iNbPossibleValues ++;
                            guint l;
                            for (l = 0; l < length; l ++)
                            {
                                ...
                                if (iValue < iNbPossibleValues)
                                {
                                    ...
                                    gtk_list_store_append (modele, 
                                                           &iter);
                                    gtk_list_store_set (modele, 
                                        &iter,
                                        CID_MODEL_ACTIVE, 
                                        TRUE,
                                        CID_MODEL_NAME, 
                                        dgettext (cGettextDomain, 
                                        pAuthorizedValuesList[iValue]),
                                        CID_MODEL_RESULT, 
                                        cValue,
                                        CID_MODEL_ORDER, 
                                        iOrder++, 
                            ...
                            // il reste des valeurs a inserer (ce peut 
                            // etre de nouvelles valeurs apparues lors 
                            // d'une maj du fichier de conf, donc 
                            // CID_WIDGET_TREE_VIEW_SORT est concerne 
                            // aussi). 
                            if (iOrder < iNbPossibleValues)  
                            {
                                gchar cResult[10];
                                for (k = 0; 
                                     pAuthorizedValuesList[k] != NULL; 
                    ...
                    break;
                    // integer
                    case CID_WIDGET_SPIN_INTEGER :  
                    // integer dans un HScale
                    case CID_WIDGET_HSCALE_INTEGER :  
                    // double integer WxH
                    case CID_WIDGET_SIZE_INTEGER :  
                        if (pAuthorizedValuesList != NULL && 
                            pAuthorizedValuesList[0] != NULL)
                            iMinValue = 
                                g_ascii_strtod(pAuthorizedValuesList[0],
                                               NULL);
                        else
                            iMinValue = 0;
                        if (pAuthorizedValuesList != NULL && 
                            pAuthorizedValuesList[1] != NULL)
                            iMaxValue = 
                                g_ascii_strtod(pAuthorizedValuesList[1],
                                               NULL);
                        else
                            iMaxValue = 9999;
                        ...
                        length = 0;
                        iValueList = 
                            g_key_file_get_integer_list (pKeyFile, 
                                                         cGroupName, 
                                                         cKeyName, 
                                                         &length, 
                                                         NULL);
                        ...
                        for (k = 0; k < iNbElements; k ++)
                        {
                            ...
                            if (iElementType == 
                                CID_WIDGET_HSCALE_INTEGER)
                            {
                                pOneWidget = gtk_hscale_new (
                                        GTK_ADJUSTMENT (pAdjustment));
                                gtk_scale_set_digits (
                                        GTK_SCALE (pOneWidget), 
                                        0);
                                gtk_widget_set (pOneWidget, 
                                                "width-request", 
                                                150, 
                                                NULL);
                            }
                            ...
                            // le 'width-request' sur un GtkHScale avec
                            // 'fMinValue' non nul plante ! 
                            // Donc on les met apres...
                            g_object_set (pAdjustment, 
                                          "lower", 
                                          (double) iMinValue, 
                                          "upper", 
                                          (double) iMaxValue, 
                            ...
                            // on rajoute le separateur.
                            if (iElementType == 
                                CID_WIDGET_SIZE_INTEGER && 
                                k+1 < iNbElements)  
                            {
                            ...
                            if (iElementType == 
                                CID_WIDGET_SIZE_INTEGER && 
                                (k&1))  // on lie les 2 spins entre eux.
                            {
                                ...
                                g_signal_connect (G_OBJECT (pOneWidget), 
                                    "value-changed", 
                                    G_CALLBACK(_cid_set_value_in_pair), 
                                    data);
                                _allocate_new_buffer;
                                data[0] = pOneWidget;
                                data[1] = pToggleButton;
                                g_signal_connect (
                                    G_OBJECT (pPrevOneWidget), 
                                    "value-changed", 
                                    G_CALLBACK(_cid_set_value_in_pair), 
                                    data);
                            }
                    ...
                    break;
                    // float.
                    case CID_WIDGET_SPIN_DOUBLE :  
                    // float avec un bouton de choix de couleur.
                    case CID_WIDGET_COLOR_SELECTOR_RGB :  
                    // float dans un HScale.
                    case CID_WIDGET_HSCALE_DOUBLE :  
                        //g_print ("  + float\n");
                        length = 0;
                        fValueList = 
                            g_key_file_get_double_list (pKeyFile, 
                                                        cGroupName, 
                                                        cKeyName, 
                                                        &length, 
                                                        NULL);
                        for (k = 0; k < iNbElements; k++) 
                        {
                            ...
                            if (pAuthorizedValuesList != NULL && 
                                pAuthorizedValuesList[0] != NULL)
                                fMinValue = g_ascii_strtod (
                                               pAuthorizedValuesList[0],
                                               NULL);
                            else
                                fMinValue = 0;
                            if (pAuthorizedValuesList != NULL && 
                                pAuthorizedValuesList[1] != NULL)
                                fMaxValue = g_ascii_strtod (
                                               pAuthorizedValuesList[1],
                                               NULL);
                            else
                                fMaxValue = 9999;

                            GtkObject *pAdjustment = 
                                gtk_adjustment_new (fValue,
                                    0,
                                    1,
                            ...
                            {
                                pOneWidget = gtk_spin_button_new (
                                    GTK_ADJUSTMENT (pAdjustment),
                                    1.,
                                    3);
                            }
                            // le 'width-request' sur un GtkHScale avec
                            // 'fMinValue' non nul plante ! Donc on les
                            // met apres...
                            g_object_set (pAdjustment, 
                                          "lower", 
                                          fMinValue, 
                            ...
                            length > 2) 
                        {
                            ...
                            gtk_box_pack_start (GTK_BOX (pHBox),
                                ...
                                FALSE,
                                0);
                            g_signal_connect(G_OBJECT (pColorButton), 
                                             "color-set", 
                                             G_CALLBACK(_cid_set_color),
                                             pSubWidgetList);
                            g_signal_connect(G_OBJECT (pColorButton), 
                                             "clicked", 
                                             G_CALLBACK(
                                                _cid_get_current_color),
                                             pSubWidgetList);
                        }
                        g_free (fValueList);
                    break;
                    ...
                    case CID_WIDGET_VIEW_LIST :
                        cValue = g_key_file_get_string (pKeyFile, 
                                                        cGroupName, 
                                                        cKeyName, 
                                                        NULL);
                        modele = s_pRendererListStore;
                        pOneWidget = gtk_combo_box_new_with_model (
                                            GTK_TREE_MODEL (modele));
                        rend = gtk_cell_renderer_text_new ();
                        gtk_cell_layout_pack_start (
                                            GTK_CELL_LAYOUT(pOneWidget),
                                            rend, 
                                            FALSE);
// model name
...
// box margin
                        ...
                        if (_cid_find_iter_from_name (modele, 
                                                      cValue, 
                                                      &iter))
                            gtk_combo_box_set_active_iter (
                                            GTK_COMBO_BOX (pOneWidget), 
                                            &iter);
                        ...
                        g_signal_connect (
                            G_OBJECT (pButtonConfigRenderer),
                            "clicked",
                    ...
                    case CID_WIDGET_DESKLET_DECORATION_LIST_WITH_DEFAULT :
                        cValue = g_key_file_get_string (pKeyFile, 
                                                        ...
                                                        cKeyName, 
                                                        NULL);
                        modele = (iElementType == CID_WIDGET_DOCK_LIST ?
                                            s_pDecorationsListStore : 
                                            s_pDecorationsListStore2);
                        pOneWidget = gtk_combo_box_new_with_model (
                                            GTK_TREE_MODEL (modele));
...
// Model Name
                    ...
                    // string
                    case CID_WIDGET_STRING_ENTRY :  
                    // string avec un selecteur de fichier a cote du 
                    // GtkEntry.
                    case CID_WIDGET_FILE_SELECTOR :  
                    // string avec un selecteur de fichier a cote du 
                    // GtkEntry et un boutton play.
                    case CID_WIDGET_SOUND_SELECTOR :  
                    // string avec un selecteur de repertoire a cote du
                    // GtkEntry.
                    case CID_WIDGET_FOLDER_SELECTOR :  
                    // string, mais sans pouvoir decochez les cases.
                    ///case CID_WIDGET_TREE_VIEW_SORT :  
                    // string, mais avec un GtkComboBoxEntry pour le 
                    // choix unique.
                    case CID_WIDGET_LIST_WITH_ENTRY : 
                    // string, avec un label pour la description. 
                    case CID_WIDGET_THEME_SELECTOR : 
                    // string avec un selecteur de font a cote du
                    // GtkEntry. 
                    case CID_WIDGET_FONT_SELECTOR :  
                    // string representee par son numero dans une liste
                    // de choix.
                    case 'r' :  
                    // string, avec un label pour la description et un
                    // bouton configurer (specialement fait pour les
                    // modules).
                    case CID_WIDGET_JUMP_TO_MODULE_IF_EXISTS :  
                    // string avec un selecteur de touche clavier
                    // (Merci Ctaf !)
                    case CID_WIDGET_SHORTKEY_SELECTOR :  
                    // string type password
                    case CID_WIDGET_PASSWORD_ENTRY :  
                        //g_print ("  + string (%s)\n", cUsefulComment);
                        pEntry = NULL;
                        ...
                        length = 0;
                        GdkPixbuf *pixbuf;
                        cValueList = g_key_file_get_locale_string_list (
                                            pKeyFile, 
                                            cGroupName, 
                                            cKeyName, 
                                            NULL, 
                        ...
                        if (iNbElements == 1) 
                        {
                                ...
                                pAuthorizedValuesList[0] == NULL) 
                            {
                                ...
                                // password mode
                                if (iElementType == 
                                    CID_WIDGET_PASSWORD_ENTRY ) 
                                {
                                    gtk_entry_set_visibility(
                                                GTK_ENTRY (pOneWidget), 
                                                FALSE);
                                    /*
...
// Model Name
                                ...
                                gchar *cResult = (iElementType == 'r' ? 
                                                    g_new0 (gchar, 10) :
                                                    NULL);
                                int ii;
                                int iNbElementsByItem = 
                                    (iElementType == 
                                    CID_WIDGET_THEME_SELECTOR ? 
                                    3 : 
                                    (iElementType == 
                                    CID_WIDGET_JUMP_TO_MODULE_IF_EXISTS?
                                        4 : 
                                        1));
                                while (pAuthorizedValuesList[k] != NULL) 
                                {
                                    ...
                                    {
                                        if (pAuthorizedValuesList[k+ii] 
                                            == NULL) 
                                        {
                                            cid_warning (
"bad conf file format, you can try to delete it and restart cid");
                                            break;
                                        }
                                    }
